...
const ProgramDetails = ({ data }) => {
  ...
  const displayLabel = detailLabels[detail] || detail // Use mapped label or fallback to original

  return (
    <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} className="mb-4">
      <motion.div
        whileHover={{ scale: 1.01 }}
        className={`flex items-center justify-between p-5 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md cursor-pointer border border-gray-100 dark:border-gray-700 transition-all duration-300 ${
          selectedDetail === detail ? "bg-gradient-to-r from-[#c3965d]/5 via-[#eabf91]/5 to-[#c3965d]/5" : ""
        }`}
      ...
      >
        <span className="text-base font-medium capitalize flex items-center gap-4 text-gray-900 dark:text-white">
          <span className="bg-gradient-to-r from-[#c3965d]/10 via-[#eabf91]/10 to-[#c3965d]/10 p-3 rounded-full">
            {icons[detail] || icons.default} {/* Use icon or fallback */}
          </span>
          <span className={`${dmsans.className} font-light tracking-wide leading-relaxed text-base text-left md:text-lg lg:text-xl`}>
            {displayLabel}
          </span>
        </span>
      ...
      {selectedDetail === detail && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="p-5 md:p-6 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700">
          {Array.isArray(data[selectedDetail]) ? (
            <div className="grid gap-4 md:grid-cols-2">
              {data[selectedDetail].map((item, idx) => (
                <div key={idx} className="bg-gray-50 font-medium dark:bg-gray-700 p-4 md:p-5 rounded-lg hover:shadow-md transition-all duration-300">
                  ...
                  <p className={`${dmsans.className} font-light tracking-wide text-base md:text-base leading-relaxed text-gray-700 dark:text-gray-300 mt-2`}>
                    {item.subheading}
                  </p>
                </div>
              ))}
