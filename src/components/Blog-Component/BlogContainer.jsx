...
const BlogContainer = ({ title, description, imageSrc, reverse, categories = false, commentCount = 0 }) => (
  <div className='flex flex-col justify-center items-center w-full'>
    <div className={clsx(
      "flex flex-col-reverse md:flex-row border-2 items-start py-0 md:py-10 w-full px-0 pb-8 mx-4 md:px-6 gap-8 md:max-w-5xl border-b",
      ...
      <div className="md:w-1/2 flex items-center justify-center z-10">
        <div className="w-full space-y-1">
          <h2 className={`${montserrat.className} text-2xl hidden md:flex font-semibold mb-2 text-gray-800 dark:text-white uppercase`}>{title}</h2>
          <div
            className={clsx(
              "bg-opacity-70  bg-gradient-to-r from-[#c3965d] via-[#eabf91] to-[#c3965d]  opacity-90 md:dark:bg-opacity-80 text-center mx-4 p-4 md:p-6 rounded-lg shadow-lg",
              "relative md:mr-10 md:max-w-[90%]",
              reverse ? "md:-left-20" : "md:-right-28"
            )}
          >
            {categories && <h3 className='md:text-sm text-sm text-center md:text-start font-semibold mb-4 text-black   dark:text-white uppercase'>{categories}</h3>}
            <p className={`${cormorant.className} text-black-600 text-lg md:text-2xl md:text-start text-black dark:text-gray-300`}>{description}</p>
          </div>
        </div>
      </div>

      {/* Image Section */}
      <div className="md:w-1/2 relative w-full h-[250px] md:h-[250px]">
        <Image
          src={imageSrc}
          alt={title}
          title={title}
