...
const courseDetailsSchema = z.object({
    ...
    courseDetail: z.array(courseDetailSchema).min(1, "At least one course detail is required"),
});
...
const serviceSchema = z.object({
    id: z.number().min(1, "Service ID is required for the update"),
    heading: z.string().min(1, "Location heading is required"),
    subheading: z.string().min(1, "Location subheading is required"),
...
const translateZodErrors = (errors) => {
    const userFriendlyMessages = {
        "id": "Service ID is required for the update.",
        ...
        categoryId: "Category is required. Please select a valid category.",
        ...
        "courseDetails.courseDetail": "The course detail is required.",
...
// PUT request handler (Update service)
export async function POST(req) {
    try {
        const data = await req.json();

        // Ensure that the ID is provided and correctly passed
        if (!data.id) {
            return NextResponse.json(
                { success: false, message: "Service ID is required for the update." },
                { status: 400 }
            );
        }
        ...
        // Check if the service exists in the database by ID
        const serviceExists = await prisma.service.findUnique({
            where: { id: parseInt(parsedData.id) }, // Ensure the ID is correct
        });

        if (!serviceExists) {
            return NextResponse.json(
                { success: false, message: "The specified service was not found. Please check the service ID." },
                { status: 404 }
            );
        }

        // Check if the category exists in the database using categoryId
        const categoryExists = await prisma.category.findUnique({
            where: { id: parseInt(parsedData.categoryId) }, // Correct field: categoryId
        });
        ...
        const updatedService = await prisma.service.update({
            where: { id: parseInt(parsedData.id) },
            data: {
                ...
                category: {
                    connect: { id: parseInt(parsedData.categoryId) }, // Correct field: categoryId
                },
            },
        });
